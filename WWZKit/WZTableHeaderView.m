...
@implementation WZTableHeaderView
/**
 *  为只读属性生成成员变量
 */
...
+ (instancetype)wwz_headerViewWithTabelView:(UITableView *)tableView{
    ...
    static NSString *reuseIdentifier = @"reuseHeaderFooterCellID";
    
    WZTableHeaderView *headerView = [tableView dequeueReusableHeaderFooterViewWithIdentifier:reuseIdentifier];
    if (!headerView) {
...
- (void)tap:(UITapGestureRecognizer *)tap{
    ...
    if ([self.delegate respondsToSelector:@selector(didTapHeaderView:)]) {
...
- (void)setReusableModel:(WZReusableModel *)reusableModel{
