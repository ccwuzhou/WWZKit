...
//  KJD
//
//  Created by appple on 15/9/2.
//  Copyright (c) 2015年 WL. All rights reserved.
//

#import "WZMenuPopover.h"
...
@interface WZMenuCell : UITableViewCell
...
+ (instancetype)wwz_menuCellWithTableView:(UITableView *)tableView{
    ...
    WZMenuCell *cell = [tableView dequeueReusableCellWithIdentifier:reuseIdentifier];
    if (!cell) {
...
#pragma mark - WZMenuPopover

@interface WZMenuPopover ()<UITableViewDataSource,UITableViewDelegate>
...
- (void)wwz_setInitContentValue{
    ...
    CGFloat space = kScreenW == 414.0 ? 10 : 5;
    
    self.anchorPoint = CGPointMake(kScreenW - space - 39*0.5, 64.0);
    
    self.cellHeight = 45.0;
    
    self.pointerFrame = CGRectMake(self.frame.size.width-18-_cornerRadius*2, 0, 18, 10);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    WZMenuCell *cell = [WZMenuCell wwz_menuCellWithTableView:tableView];
...
- (void)wwz_show{
    ...
    backView.backgroundColor = kColorFromRGBA(0, 0, 0, 0.2);
